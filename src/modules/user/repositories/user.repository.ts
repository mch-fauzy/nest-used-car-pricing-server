...
export class UserRepository {
  ...
  async findById(primaryId: Pick<User, 'id'>): Promise<User | null> {
    const user = await this.userRepo.findOneBy(primaryId);
    return user;
  }

  async deleteById(primaryId: Pick<User, 'id'>) {
  ...
  async updateById(primaryId: Pick<User, 'id'>, data: Omit<User, 'id'>) {
    const user = await this.findById(primaryId);
